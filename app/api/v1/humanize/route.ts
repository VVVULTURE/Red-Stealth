...
export async function POST(request: NextRequest) {
  ...
  const provider = payloadModel || process.env.STEALTHHUMANIZER_PROVIDER || 'rudra-free';
  const providerApiKey = payloadApiKey || process.env.STEALTHHUMANIZER_PROVIDER_API_KEY;
  if (provider !== 'rudra-free' && !providerApiKey) {
