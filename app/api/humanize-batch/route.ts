...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isRudraFree && !isRudraHumanizerConfigured()) {
      return NextResponse.json({ success: false, error: "Rudra's Free Usage Model is not configured on this deployment. Set RUDRA_HUMANIZER_API_KEY or choose another provider." }, { status: 503 });
    }
    ...
    const results = await asyncMapConcurrent(
      ...
      async (text: unknown, index: number) => {
        ...
        try {
          const raw = isRudraFree
            ? (await humanizeWithRudra(text.trim())).text
            : await generateWithProvider(model, apiKey, systemPrompt, text.trim(), {
