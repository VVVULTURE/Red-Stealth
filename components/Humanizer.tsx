...
export default function Humanizer({ showToast }: HumanizerProps) {
  ...
  // The built-in provider is the website's no-key default. Other providers
  // must be explicitly selected and configured by the user.
  const DEFAULT_PROVIDER: ModelProvider = 'rudra-free';
              ...
              <div className="border-t border-dark-700/30 pt-4 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => {
                      ...
                      // Do not silently choose a provider. Users can select
                      // the chain models they have configured below.
                      if (!newVal) setSelectedChainModels([]);
                    }}
                ...
                {enableChain && (
                  <div className="space-y-2 animate-fade-in">
                    <p className="text-xs text-dark-400">Select models to chain through. Providers that require keys must be configured in Settings:</p>
