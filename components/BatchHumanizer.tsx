...
export default function BatchHumanizer({ showToast }: BatchHumanizerProps) {
  ...
  const [model, setModel] = useState<ModelProvider>(PROVIDERS[0]?.id ?? 'rudra-free');
  ...
  const handleSubmit = async () => {
    ...
    if (!apiKey && model !== 'rudra-free') {
      showToast('error', 'This provider needs an API key. Add one in Settings or choose Rudra’s Free Usage Model.');
      return;
    }
